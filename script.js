...
function renderMapOverlays() {
  mapOverlays.innerHTML = Object.entries(OVERLAY_POSITIONS).map(([tableName, pos]) => `
    <a
      class="map-button"
      href="#${tableId(tableName)}"
      title="${escapeHTML(tableName)}"
      aria-label="Open ${escapeHTML(tableName)} projects"
...
function renderProjects() {
  ...
  grid.innerHTML = filtered.map(project => {
    ...
    const tableName = Object.entries(TABLE_LAYOUT).find(([_, nums]) => nums.includes(project.projectNumber))?.[0] || "Unassigned";
