...
const prefixNames = {
  ...
  R: "Neural Circuits: Research and Innovation Society",
  T: "Career and Technical Education (CTE)"
};
...
function safeId(projectNumber) {
  return `project-${String(projectNumber).replace(/[^a-zA-Z0-9_-]/g, "")}`;
}

function prefixOf(projectNumber) {
  return String(projectNumber || "").trim().charAt(0).toUpperCase();
}

function renderSubjectSummary() {
  const counts = PROJECTS.reduce((acc, p) => {
    const prefix = prefixOf(p.projectNumber);
    const key = p.subject || prefixNames[prefix] || "Other";
        ...
        ${row.map((table, tableIndex) => `
          <div class="table-card">
            ...
            <div class="table-booths">
              ${table.map(number => {
                if (number === "EMPTY") {
                  return `<div class="booth empty">Empty</div>`;
                }
...
function renderProjects() {
  ...
  grid.innerHTML = filtered.map(project => {
    ...
    return `
      <article class="project-card" id="${safeId(project.projectNumber)}">
        <div class="meta">
